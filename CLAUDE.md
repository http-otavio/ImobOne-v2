...
| `sellers-ai-dossier` | Dossiê de Captação e Posicionamento de Mercado | critical | Dono | ✅ done |
| `permuta-triage` | Fluxo de Qualificação Complexa de Permutas | critical | Dono/Corretor | ✅ done |
| `liquidity-yield-dossier` | Expansão Hard Skills do Dossiê de Caviar | critical | Dono | ✅ done |
...
*Última atualização: Abril 2026 — permuta-triage: tools/permuta.py (detect_permuta com 8+ padrões regex — linguagem formal e implícita de alto padrão; extract_permuta_data via Claude Haiku extrai tipo/bairro/valor/características do ativo; calculate_permuta_score_bonus — +3 pts padrão para ativo >= R$500k, configurável via onboarding.json['permuta_score_bonus']; format_permuta_briefing_section — seção 'Análise de Permuta' no briefing do corretor; save_permuta_data via Supabase PATCH). whatsapp_webhook.py: _check_and_process_permuta fire-and-forget após score de intenção; seção de permuta injetada automaticamente em _notify_corretor quando permuta_detectada=true no Redis; score bonus acumulado no Redis. _prompts_build/consultant_base.md: CENÁRIO 10 — PERMUTA com coleta sequencial de dados do ativo (1 pergunta/turno), proibições e notificação ao corretor. Supabase migration: add_permuta_fields (permuta_detectada BOOLEAN, permuta_dados JSONB, permuta_detectada_em TIMESTAMPTZ + índice). 217 testes passando no VPS (25 novos permuta + 192 anteriores). backlog/tasks.json: permuta-triage marcado done.*
*Última atualização: Abril 2026 — sellers-ai-dossier: tools/sellers_dossier.py (get_luxury_pois via Google Places API — 8 categorias premium, filtro rating>=4.0, fallback gracioso sem key; get_comparable_properties via Supabase pgvector metadata filter + fallback gracioso; generate_captacao_markdown via Claude Sonnet — 5 seções obrigatórias; render_captacao_pdf via reportlab layout header dark/gold; save_captacao_locally em clients/{client_id}/assets/; send_captacao_to_corretor via Evolution API sendMedia document; generate_sellers_dossier orquestrador — falha no envio tolerável). tests/test_sellers_dossier.py: 17/17 passando. 234 testes totais no VPS. backlog/tasks.json: sellers-ai-dossier marcado done.*
*Última atualização: Abril 2026 — liquidity-yield-dossier: tools/liquidity.py (buscar_dados_liquidez com fallback estruturado FipeZap Q4 2024 — 7 bairros SP/RJ, suporte a dados customizados por cliente via clients/{id}/liquidity_data.json, match case-insensitive + parcial, retorna None quando sem fonte verificável — nunca inventa). format_metricas_financeiras com linha FONTE OBRIGATÓRIA. tools/dossie.py: seção 'Métricas Financeiras Verificadas' renderizada no PDF (reportlab kv_table + disclaimer), parâmetro liquidity_data em build_and_send_dossie + generate_dossie_content, prompt com CRÍTICO contra invenção de dados financeiros, metricas_financeiras: null no schema. whatsapp_webhook.py: _generate_and_send_dossie detecta perfil investidor via lead_profiles.purchase_purpose + neighborhoods, chama buscar_dados_liquidez fire-and-forget, passa liquidity_data ao dossiê (seção omitida quando None). tests/test_liquidity_dossier.py: 16/16 passando. 255 testes passando no VPS (16 novos + 239 anteriores). backlog/tasks.json: liquidity-yield-dossier marcado done.*
