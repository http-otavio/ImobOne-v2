...
- **Detecção de objeção — regex primeiro, Haiku fallback:** ordem de categorias no `_PATTERNS` importa — `condominio` e `concorrente` antes de `preco` evitam falsos positivos do padrão genérico "caro". Haiku skippado silenciosamente sem `ANTHROPIC_API_KEY`. Deduplicação: mesma categoria não registrada mais de uma vez por dia por lead. Fire-and-forget via `asyncio.create_task` após score de intenção.
