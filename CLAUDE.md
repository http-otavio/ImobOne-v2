...
*Última atualização: Abril 2026 — pipeline-roi-calc implementado: migration Supabase (pipeline_value_brl, pipeline_imovel_ids, pipeline_updated_at), _update_pipeline_value() no webhook (fire-and-forget após dispatch_photos), 15 testes. 101 testes totais passando (15 pipeline + 50 portal + 36 report). portal_lead_capture.py deployado (ZAP/VivaReal/OLX, 50 testes). Report Engine semanal executivo (36 testes). Timer imob-weekly-report.timer ativo — domingo 21h BRT. Nightly Squad executou 02:00 de 13/04 — 3 bugs corrigidos. WhatsApp devlabz state: open. Pipeline Runner ativo (porta 8003). Fase 3 mapeada. CRM 6 adapters. Backlog: 11 tasks. Pendente: 360dialog para primeiro cliente real.*
